...
import type { FloatingModule } from "../../common/abstracts/base_floating_module";
import { Page } from "../../common/abstracts/base_page";

import { moduleEnum } from "../../common/enum/modules_enum";
import type { ExerciceConfigDTO } from "../../common/types/app-config";
import type { Corner, Dimensions, Edge, ModuleDTO } from "../../common/types/floating_module";
import type { Song } from "../../common/types/playlist";
import { PlaylistDAO } from "../../core/data_acess/playlist_indexedDB_dao";
import { ExerciceStore } from "../../core/state_management/exercice_state_store";
import { Spinner } from "../../ui_components/_base/spinner/spinner";
import { ExerciceHeader } from "./components/exercice_header_component/exerciceHeader";
import { ExerciceMagnet } from "./components/exercice_magnet_component/exerciceMagnet";
import { ExerciceMenu } from "./components/exercice_menu_component/exerciceMenu";
export class Exercice extends Page {
  ...
  public modules: Map<number, FloatingModule> = new Map();
  private topZIndex = 0;
  public draggedModule: FloatingModule | null = null;
  public resizedModule: { module: FloatingModule; edge: Edge | Corner }[] = [];
  ...
  private async init() {
    if (!this.params || !this.params.id) {
      AppRunner.getInstance().router?.redirect("not-found");
      return;
    }
  ...
  private addModules() {
    ...
    this.song.exercice_config.modules.forEach((m) => {
      const ModuleClass = moduleEnum[m.type];
  ...
  public removeModule(module: FloatingModule) {
  ...
  public changeDraggedModule(module: FloatingModule | null) {
    if (module) {
  ...
  public bringModuleToFront(module: FloatingModule) {
    this.topZIndex += 2;
