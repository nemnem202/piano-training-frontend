...
import type { Exercice } from "../../features/exercice/exercice_page";
import type { Bounds, Corner, Edge, ModuleDTO, Position } from "../types/floating_module";
import { Component } from "./base_component";

const RESIZE_TOLERANCE = 10;
const CLOSE_BUTTON_WIDTH = 50;

export abstract class FloatingModule extends Component {
...
export interface MoveStrategy {
  getPosition(x: number, y: number, module: FloatingModule): { x: number; y: number };
}

export interface ResizeStrategy {
  getSize(width: number, height: number, module: FloatingModule): { width: number; height: number };
}

export class MagnetismMoveStrategy implements MoveStrategy {
  getPosition(x: number, y: number, module: FloatingModule) {
...
export class MagnetismResizeStrategy implements ResizeStrategy {
  getSize(width: number, height: number, module: FloatingModule) {
